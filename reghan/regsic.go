...
type MakeRespBase[RES any, RESPONSE any] func(res RES, erx error) *RESPONSE //使用基本类型做返回值
...
func Handle1a[ARG, RES any, RESPONSE any](run Handle1aFunc[ARG, RES], parseReq ParseReqFunc[ARG], respFunc MakeRespBase[RES, RESPONSE]) gin.HandlerFunc {
	return func(ctx *gin.Context) {
		...
		if erx != nil {
			var res RES // zero
			ctx.SecureJSON(http.StatusOK, respFunc(res, erero.WithMessage(erx, "PARAM IS WRONG")))
			return
		}
